...
export const createMarkerSVG = (type: 'location' | 'locationSecondary' | 'player' | 'active' | 'activeSecondary'): string => {
    ...
    // Special override for player icon to show a Google Maps-style person marker
    if (type === 'player') {
        return `
        <svg viewBox="0 0 36 48" width="36" height="48" xmlns="http://www.w3.org/2000/svg">
            ...
            <!-- Google Maps-style person in a white circle -->
            <circle cx="18" cy="16" r="10.5" fill="#ffffff" opacity="0.98"/>
            <g fill="${c.bg}">
                <circle cx="18" cy="13" r="3.2"/>
                <path d="M12.5 22.5c0-3 2.7-5.5 5.5-5.5s5.5 2.5 5.5 5.5v1.5H12.5z"/>
            </g>
        </svg>
    `;
    }
...
export const createVintageIcon = (
    ...
    // Use questRedPulse for active markers to pulsate and turn red
    const flashAnimation = isActive ? 'animation:questRedPulse 2s ease-in-out infinite;' : '';

    return new DivIcon({
        className: '',
        html: `<div style="position:relative;width:36px;height:48px;${flashAnimation}transform-origin:center bottom;">${createMarkerSVG(type)}${labelHtml}${isActive ? `<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:50px;height:50px;border:2px solid #FF0000;border-radius:50%;animation:questPulse 2s ease-out infinite;pointer-events:none;"></div>` : ''
            }</div>`,
        iconSize: [36, 48],
        iconAnchor: [18, 48],
        popupAnchor: [0, -48]
