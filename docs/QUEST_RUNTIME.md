...
**Trigger**: Player app loads (`page.tsx` → `useQuestSession`)
...
1. `useProximityTracker` → `handleEnterZone` (QuestMap.tsx)
2. `handleEnterZone` → `handleObjectArrival` (useArrivalSimulation.ts)
3. `handleObjectArrival` → `runtime.arriveAtObject(obj.id)`
...
```json
{
  ...
  "snapshot": {
    ...
    "nodes": {
      ...
      "tl_kitchen-001:audio-intro": { "status": "unlocked" } // ← Timeline starts
    }
  },
  "deltas": [
...
const runObjectTimeline = async (obj: QuestObject) => {
  ...
  for (let idx = progress.nextIndex; idx < timeline.items.length; idx++) {
    ...
    // → Calls POST /api/runtime/node/complete
...
useEffect(() => {
  socket.on('quest_runtime_deltas', (message) => {
    // Dispatch window event → triggers useQuestRuntime refresh
...
- [x] Quest compiler (`quest.json` → `compiled.json`)
...
- [ ] End-to-end integration testing (Next.js client ↔ AWS backend)
...
**Problem**: After completing a puzzle and returning to the map, the timeline would not resume and continue to the next items (video → audio → __end).
...
**Flow**:
```
PuzzleClient → fetch('/api/runtime/puzzle/submit') → Server updates DynamoDB ✅
                                                    → Response with snapshot ❌ IGNORED
QuestMap → Uses stale snapshot → Timeline can't detect completed puzzle ❌
```
...
**Flow**:
1. Player navigates to puzzle page → timeline breaks with `timelineRunRef = { objectId, version, cancel: false }`
...
4. Player completes puzzle and navigates back → QuestMap calls `runObjectTimeline`
...
4. Continues through all remaining timeline items (video → audio → __end)
...
for (const item of items) {
  ...
  if (item.repeatPolicy !== 'once_per_object') continue;  // ❌ Conditional
...
for (const item of items) {
  ...
  const nodeId = makeTimelineItemNodeId(objectId, item.key);
  const node = snapshotRef.current?.nodes?.[nodeId];
  if (node?.status === 'completed') {  // ✅ No conditionals
    completedKeys[item.key] = true;
  }
}
```
...
**Deployment**:
1. Pushed to GitHub → GitHub Actions deployed Lambda with updated Python compiler
...
**Root Cause**: The timeline runner processed all items correctly (audio → streaming text → puzzle → video → final audio), but after the last item completed, the timeline loop exited **without completing the `__end` node**.
...
**Server-Side Flow**:
1. Frontend completes `__end` node → `POST /api/runtime/node/complete`
...
**Check**:
1. Network tab: Look for `POST /api/runtime/object/arrive` → should return `NODE_UNLOCKED` deltas
2. Console: `[QuestRuntime]` logs show snapshot with unlocked nodes
3. React DevTools: Check `useQuestRuntime` → `snapshot.nodes` → verify node status is `unlocked`
